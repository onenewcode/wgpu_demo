...
impl Example {
    // 这个函数的目的是生成一个用于三维图形渲染的变换矩阵，它考虑了透视投影和相机的位置和方向。这个矩阵可以用于将三维空间中的点转换到二维屏幕上的位置。
...
impl crate::framework::Example for Example {
    ...
    fn init(
        ...
        let texture_extent = wgpu::Extent3d {
            width: size,
            height: size,
            depth_or_array_layers: 1, //设置纹理层数
        };
        let texture = device.create_texture(&wgpu::TextureDescriptor {
            label: None,
            size: texture_extent,
            mip_level_count: 1, // Mipmap 是一系列预计算的、逐渐缩小的图像副本，用于提高渲染效率。这里设置为 1 表示不使用 Mipmap。
        ...
        // 用于存储 uniforms 数据。uniforms 是在图形渲染中用于传递常量数据到着色器程序的一种方式，
