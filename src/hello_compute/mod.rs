...
async fn execute_gpu_inner(
    ...
    // 创建一个暂存缓冲区 (staging_buffer)，它用于在 GPU 计算完成后将数据复制回 CPU。该缓冲区具有 MAP_READ 和 COPY_DST 的使用标志。
    let staging_buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: None,
        size,
        usage: wgpu::BufferUsages::MAP_READ | wgpu::BufferUsages::COPY_DST, // 如何使用
    ...
    // 创建一个存储缓冲区 (storage_buffer)，并将 numbers 数据初始化到这个缓冲区中。该缓冲区可以作为存储缓冲区绑定到着色器，并且支持复制操作。
    ...
    // 创建一个计算管线 (compute_pipeline)，它指定了使用的着色器模块和入口点 (main)。
    let compute_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: None,
        layout: None,
        module: &cs_module, //计算逻辑
        entry_point: "main",
